#!/usr/bin/env python3
"""g1_rgmt_tracking_global 的键盘操作台：状态切换和动作切换都在这里按键完成。
...
from g1_rgmt_tracking_global.rgmt_runtime import resolve_policy_path
...
第一次上机务必吊装。本策略依赖里程计，status 里出现 CLAMPED 就是估计已不可信，
立即急停；offset 持续增大说明真的跟不上了。
"""


class RgmtTeleop(Node):

    def __init__(self) -> None:
        super().__init__('rgmt_teleop')
        target = self.declare_parameter('tracking_node', '/rgmt_tracking') \
...
def main(args=None) -> None:
    if not sys.stdin.isatty():
        raise SystemExit('操作台要在终端里跑：ros2 run g1_rgmt_tracking_global teleop_keyboard')
