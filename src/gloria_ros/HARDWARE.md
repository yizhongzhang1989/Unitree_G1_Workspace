# Gloria-M 硬件与协议技术文档
本文总结 Synria Robotics 云犀 Gloria-M 平行二指夹爪的机械接口、电气接口、CAN 协议、设备寄存器、标定方法与安全边界。本文依据以下资料整理：

- [Gloria-M 官方产品介绍](https://docs.sparklingrobo.com/docs/gloria-m-series)
- [官方安装与使用指南](https://docs.sparklingrobo.com/docs/gloria-m-series/doc_01_installation_guide)
- [官方通信协议](https://docs.sparklingrobo.com/docs/gloria-m-series/protocol/doc_00_intro)
- [官方 Python SDK 文档](https://docs.sparklingrobo.com/docs/gloria-m-series/doc_02_sdk_communication)
- [`Gloria-M-SDK`](../../sdk/Gloria-M-SDK/README.zh-CN.md) 源码

整理日期：2026-07-11

> **资料边界**：Gloria-M 硬件使用 24 V DC 供电（问淘宝客服得到的；已经实测检验过）。官网公开页面尚未给出峰值电流、重量、连接器针脚定义、额定夹持力或完整安装孔位表。接线和上电前仍须核对设备铭牌、随货资料或厂家确认值，不能从 SDK 默认参数或线缆颜色反推其余电气规格。

## 1. 机械尺寸
官网公开的 100 mm 夹爪尺寸图：

![Gloria-M 100 mm 夹爪尺寸图](https://download.sparklingrobo.com/avatar/hZzTGOjQUWnw6OZoyqf8EgJxvM0ufgimage/png)

*图 1：Gloria-M 100 mm 夹爪尺寸，来源：[官方产品介绍](https://docs.sparklingrobo.com/docs/gloria-m-series)。*

图中标注如下，单位均为 mm。尺寸是图示值，不代表公差、安装孔位或碰撞包络已经
完整定义。

| 项目 | 图示值 | 说明 |
|---|---:|---|
| 指间内开口 | 100 | 图示张开状态两指内侧间距 |
| 图示总宽 | 157 | 图示张开状态外廓宽度 |
| 图示总高 | 130 | 从指端至上部电机/驱动组件 |
| 指端宽度 | 43 | 单个指端沿图示纵向的宽度 |
| 中心圆弧 | R28.5 | 中央机构标注半径 |

## 2. 电气接口与通信路径
夹爪由独立的 24V DC 电源供电。官方接线图标出了电源、CAN 与串口调试接口：

![Gloria-M 接口与接线图](https://download.sparklingrobo.com/avatar/vqtcbKodHmtgUWnw6OZoyqf8EgJxvM0ufgimage/png)

*图 2：Gloria-M 接口与官方串口调试链路，来源：[官方安装与使用指南](https://docs.sparklingrobo.com/docs/gloria-m-series/doc_01_installation_guide)。*

| 接口 | 标注用途 | 使用说明 |
|---|---|---|
| XT30 电源接口 | 24 V DC 工作供电 | 独立给夹爪供电；接线前确认极性和电源容量 |
| XT30(2+2) 接口 | CAN 通信 | 连接符合设备电气要求的 CAN 总线 |
| GH1.25 3PIN | 串口通信 | 配合厂家串口调试链路使用 |
| Type-C/调试板 | 上位机串口转 CAN 链路 | `Gloria-M-SDK` 的 `SerialCanAdapter` 使用该类链路 |

设备可以通过原生 CAN 总线或厂家 Type-C 串口转 CAN 调试链路访问：
```text
原生 CAN：上位机 -> USB-CAN 适配器 -> XT30(2+2) CAN -> Gloria-M
调试链路：上位机 -> Type-C 串口转 CAN 调试板 -> Gloria-M
```

同一物理适配器或总线不应由多个控制程序同时写入，否则模式配置、寄存器操作和运动命令可能互相覆盖。

### 2.1 两种“波特率”不能混淆

| 参数 | 示例值 | 来源与含义 |
|---|---:|---|
| 串口波特率 | 921600 bit/s | SDK `SerialCanAdapter` 的 USB 串口速率 |
| CAN 总线比特率 | 1000000 bit/s | 本文实机验证时使用的 CAN 速率 |

`921600` 不是 CAN 比特率。`1000000` 也不应被当作所有 Gloria-M 固件的无条件出厂值。
夹爪寄存器中存在 `can_br`（RID 35），但 SDK 未给出枚举值与实际 bit/s 的换算表。接入前应使用厂家调试工具确认设备与 CAN 适配器速率一致。

### 2.2 总线接线原则
- 断电完成电源和 CAN 接线，确认电源极性后再上电。
- 不要按官方示意图中的线色猜测 XT30(2+2) 针脚，必须查实物标识或厂家针脚表。
- CAN_H、CAN_L 和参考地应正确连接，双绞线尽量短并远离电机动力线。
- 按 CAN 总线规范，仅在物理总线两端配置终端电阻，避免星形长支线。
- 同一总线的所有节点必须使用相同比特率，且活动 CAN ID 不能产生非预期冲突。
- 本文涉及的设备协议均使用标准 11-bit CAN 数据帧。

## 3. CAN 标识符规划
协议使用标准 11-bit CAN ID。`command_id` 对应设备 `ESC_ID`，`feedback_id` 对应设备 `MST_ID`。以下示例值来自 SDK 默认值和实机验证，不保证等于每台设备的出厂设置。

| 用途 | 计算/默认值 | `command_id=0x01` 示例 |
|---|---:|---:|
| MIT、使能、失能、设零命令 | `command_id` | `0x001` |
| PV 命令 | `0x100 + command_id` | `0x101` |
| 参数读写、保存、状态请求 | 固定广播 ID | `0x7FF` |
| 状态/参数反馈 | 设备 `MST_ID` | 实机验证值 `0x101` |

官网协议说明反馈帧 ID 由 `MST_ID` 设置且默认值为 0，SDK 示例通常使用 `0x101`；接入时必须读取或抓包确认实机值。不同固件的状态反馈可能出现在 `MST_ID`、`ESC_ID` 或 `0x000`。状态帧 `Data[0]` 低 4 位携带设备号，可用于进一步确认来源。

寄存器回包没有独立的 payload 设备号。多设备共总线时，建议为每台设备配置非零且唯一的 `MST_ID`，并确保 `ESC_ID` 低 4 位非零且唯一。寄存器回包的 `Data[0:2]` 可能为保留零字节，也可能回显设备命令地址；`Data[2]` 为 `0x33/0x55`。

多夹爪共总线时，应检查每台设备的完整活动 ID 集合，而不只是 `command_id`。例如：

| 设备 | `command_id` | PV ID | `feedback_id` |
|---|---:|---:|---:|
| 左夹爪 | `0x01` | `0x101` | `0x101` |
| 右夹爪 | `0x02` | `0x102` | `0x102` |

不同物理总线上的设备可以复用同一组 ID；同一物理总线上的所有活动 ID 必须避免冲突。

## 4. 控制协议
除特别说明外，控制与反馈均为 DLC 8 的标准 CAN 数据帧。

### 4.1 通用控制命令
通用命令发送到 `command_id`，数据前 7 字节均为 `0xFF`：

| 命令 | Data[7] | 上电/运行语义 |
|---|---:|---|
| 清除错误 | `0xFB` | 清除可恢复故障 |
| 使能 | `0xFC` | 上电自检后进入主动控制 |
| 失能 | `0xFD` | 停止主动驱动 |
| 设置位置零点 | `0xFE` | 将当前位置作为零点 |

上电默认处于失能状态。必须先确认控制模式和量程，再使能和发送运动命令。

SDK 还使用以下状态请求帧，发送到 `0x7FF`：

```text
Data = [command_id低字节, command_id高字节, 0xCC, 0, 0, 0, 0, 0]
```

### 4.2 MIT 阻抗/力矩模式
MIT 命令发送到 `command_id`，5 个变量压缩在 8 字节中：

| 字节 | 位布局 |
|---|---|
| 0..1 | `p_des`，16 bit |
| 2 | `v_des[11:4]` |
| 3 | `v_des[3:0] | kp[11:8]` |
| 4 | `kp[7:0]` |
| 5 | `kd[11:4]` |
| 6 | `kd[3:0] | t_ff[11:8]` |
| 7 | `t_ff[7:0]` |

固件目标力矩关系为：
$$
\tau_{ref} = k_p(p_{des}-p_{fb}) + k_d(v_{des}-v_{fb}) + \tau_{ff}
$$

协议范围：

| 量 | 范围 | 单位/说明 |
|---|---:|---|
| `p_des` | `[-PMAX, PMAX]` | rad，16 bit |
| `v_des` | `[-VMAX, VMAX]` | rad/s，12 bit |
| `kp` | `[0, 500]` | 12 bit |
| `kd` | `[0, 5]` | 12 bit |
| `t_ff` | `[-TMAX, TMAX]` | N m，12 bit |

典型组合：

- 位置阻抗：`kp > 0`、`kd > 0`，`tau = 0`。
- 速度控制：`kp = 0`、`kd > 0`，设置 `dq`。
- 力矩前馈：`kp = 0`，使用低幅值 `tau`；工程上仍建议保留适当 `kd` 抑制冲击。

官网明确警告：进行位置控制时 `kd` 不能为 0，否则可能震荡甚至失控。

### 4.3 PV 位置速度模式
PV 命令发送到 `0x100 + command_id`：
```text
Data[0:4] = p_des，IEEE-754 float32，小端序，单位 rad
Data[4:8] = v_des，IEEE-754 float32，小端序，单位 rad/s
```

`p_des` 是目标位置，`v_des` 是轨迹最大绝对速度。固件内部使用位置环、速度环和电流环。
按“最大绝对速度”的语义，`v_des` 应使用非负值且不超过设备配置的速度量程。
官网建议阻尼因子设置为非零正数；出现震荡时应同时检查阻尼、加速度和减速度设置。

### 4.4 反馈帧
所有控制模式共用 MIT 风格状态反馈：

| 字节 | 字段 | 含义 |
|---|---|---|
| 0 | `ERR[3:0] | ID[3:0]` | 高 4 位状态码，低位设备 ID |
| 1..2 | `POS` | 16-bit 位置 |
| 3..4 高 4 位 | `VEL` | 12-bit 速度 |
| 4 低 4 位..5 | `T` | 12-bit 力矩 |
| 6 | `T_MOS` | 驱动 MOS 平均温度，摄氏度 |
| 7 | `T_Rotor` | 电机线圈平均温度，摄氏度 |

官网状态码：

| `ERR` | 状态 |
|---:|---|
| `0x0` | 失能 |
| `0x1` | 使能 |
| `0x8` | 超压 |
| `0x9` | 欠压 |
| `0xA` | 过电流 |
| `0xB` | MOS 过温 |
| `0xC` | 电机线圈过温 |
| `0xD` | 通信丢失 |
| `0xE` | 过载 |

接收端必须同时解析 `ERR`、设备号、位置、速度、力矩和两项温度。发出使能命令只表示
主机提出请求，不能替代设备反馈中的 `ERR=1` 确认；运行时还应持续监控故障码和温度。

### 4.5 定点映射与量化
SDK 对位置、速度和力矩使用相同的线性映射。对范围 `[x_min, x_max]` 和 `n` 位整数：
$$
u = \left\lfloor\frac{x-x_{min}}{x_{max}-x_{min}}(2^n-1)\right\rfloor
$$
$$
x = x_{min} + \frac{u}{2^n-1}(x_{max}-x_{min})
$$

SDK 默认 `PMAX=3.14`、`VMAX=10`、`TMAX=12` 时，理论量化步长约为：

| 反馈量 | 步长 |
|---|---:|
| 位置 | `9.58e-5 rad`，约 `0.00549 deg` |
| 速度 | `4.88e-3 rad/s` |
| 力矩 | `5.86e-3 N m` |

`PMAX/VMAX/TMAX` 是主机与固件共同使用的**编解码量程**。它们不是机械安全行程，也不是建议工作载荷。任一值不一致都会导致同一比特流在主机和设备上表示不同的物理量。

### 4.6 CAN 帧发送时序示例

并非所有帧都按固定周期发送。配置、使能、清错和失能由操作事件触发；寄存器操作应当发送一帧并等待对应回包，再继续下一步；只有运动命令和运行状态检查需要周期发送。

下面以 `command_id=0x001`、`feedback_id=0x101`、PV 模式、100 Hz 运动控制为例。时间仅表示先后关系；模式和参数请求应以收到有效回包为继续条件，而不是依赖固定延时。`gloria_ros` 默认单次应答超时为 500 ms，运行状态请求周期为 100 ms。

```mermaid
sequenceDiagram
	participant H as 主机
	participant G as Gloria-M
	H->>G: 0x7FF 写 CTRL_MODE=2
	G-->>H: 0x101 模式写入回包
	H->>G: 0x7FF 读 PMAX
	G-->>H: 0x101 PMAX 回包
	H->>G: 0x7FF 读 VMAX
	G-->>H: 0x101 VMAX 回包
	H->>G: 0x7FF 读 TMAX
	G-->>H: 0x101 TMAX 回包
	H->>G: 0x001 使能 0xFC
	H->>G: 0x7FF 请求状态 0xCC
	G-->>H: 0x101 状态反馈
	loop 每 10 ms，100 Hz
		H->>G: 0x101 PV 运动命令
		G-->>H: 0x101 状态反馈（按实机固件行为）
	end
	loop 每 100 ms，10 Hz
		H->>G: 0x7FF 请求状态 0xCC
		G-->>H: 0x101 状态反馈
	end
	H->>G: 0x001 失能 0xFD
```

对应的主机发送帧如下：

| 阶段 | 发送时机 | CAN ID | Data（十六进制） | 后续条件 |
|---|---|---:|---|---|
| 选择 PV 模式 | 失能状态下，启动配置时发送一次 | `0x7FF` | `01 00 55 0A 02 00 00 00` | 等待 RID 10 的 `0x55` 回包，并确认值为 2 |
| 读取 `PMAX` | 模式确认后发送一次 | `0x7FF` | `01 00 33 15 00 00 00 00` | 等待 RID 21 的 `0x33` 回包 |
| 读取 `VMAX` | `PMAX` 确认后发送一次 | `0x7FF` | `01 00 33 16 00 00 00 00` | 等待 RID 22 的 `0x33` 回包 |
| 读取 `TMAX` | `VMAX` 确认后发送一次 | `0x7FF` | `01 00 33 17 00 00 00 00` | 等待 RID 23 的 `0x33` 回包 |
| 使能 | 模式和三个量程均确认后发送一次 | `0x001` | `FF FF FF FF FF FF FF FC` | 主动请求状态并确认反馈 `ERR=1` |
| 请求状态 | 使能后立即一次，运行中默认每 100 ms 一次 | `0x7FF` | `01 00 CC 00 00 00 00 00` | 等待有效状态反馈；不用于设置运动目标 |
| PV 运动 | 使能、模式匹配且反馈新鲜时，建议按控制周期发送 | `0x101` | `p_des` float32 + `v_des` float32 | 持续接收并检查状态反馈 |
| 失能 | 正常停止、异常退出或反馈超时时立即发送一次 | `0x001` | `FF FF FF FF FF FF FF FD` | 停止发送运动命令 |

确认反馈 `ERR=1` 是协议级安全要求。当前 `gloria_ros` 版本在使能流程中只确认收到了归属正确且可解码的状态帧，尚未解析 `ERR`、`T_MOS` 和 `T_Rotor`；在该功能补齐前，实机调试还应通过 CAN 抓包或厂家工具核对设备确实处于使能且无故障状态。

例如目标位置为 `2.5 rad`、最大速度为 `0.5 rad/s` 时，两个小端序 float32 组成的 PV 帧为：

```text
CAN ID = 0x101
Data   = 00 00 20 40 00 00 00 3F
		 |-- 2.5f --| |-- 0.5f --|
```

目标到达后若要保持位置，SDK 示例会继续周期发送相同 `p_des`，并将 `v_des` 设为 `0.0`：

```text
CAN ID = 0x101
Data   = 00 00 20 40 00 00 00 00
		 |-- 2.5f --| |-- 0.0f --|
```

MIT 模式使用相同生命周期，只需将模式值改为 1，并把周期运动帧改为发送到 `command_id` 的 MIT 压缩帧。SDK 的 PV 流式示例采用约 100 Hz，API 文档建议更新目标时使用约 100--500 Hz；实际频率还必须结合固件 `TIMEOUT`、共享总线负载和控制器实时性确定。停止上层运动消息并不等于失能，结束控制时仍须显式发送 `0xFD`。

状态反馈是否会严格跟随每一条运动命令返回，应以具体固件抓包结果为准。因此运行程序应异步接收所有反馈，同时保留低频 `0xCC` 主动状态请求作为在线检查。`0xCC` 不能替代运动帧，也不应未经验证就视为固件运动通信看门狗的喂狗帧。

其余帧均不应放入正常运动循环：`0xFB` 只在识别并排除可恢复故障原因后清错；`0xFE` 只在失能且完成机械标定时设零；寄存器保存 `0xAA` 只在确需跨掉电保留配置时发送一次。

## 5. 寄存器协议
寄存器请求均发送到标准 CAN ID `0x7FF`。SDK 实际使用的 8 字节格式为：
```text
读取：[ID_L, ID_H, 0x33, RID, 0, 0, 0, 0]
写入：[ID_L, ID_H, 0x55, RID, value0, value1, value2, value3]
保存：[ID_L, ID_H, 0xAA, 0,   0, 0, 0, 0]
```

4 字节值使用小端序。SDK 将以下 RID 解码为 `uint32`：`7..10`、`13..16`、`35..36`；其他已知参数按 `float32` 解码。

### 5.1 常用寄存器

| RID | SDK 名称 | 类型 | 含义 |
|---:|---|---|---|
| 0 | `UV_Value` | float32 | 欠压保护阈值 |
| 2 | `OT_Value` | float32 | 过温保护阈值 |
| 3 | `OC_Value` | float32 | 过流设置 |
| 4 | `ACC` | float32 | 加速度，官网单位 Krad/s² |
| 5 | `DEC` | float32 | 减速度，官网单位 Krad/s² |
| 6 | `MAX_SPD` | float32 | 理论速度限制 |
| 7 | `MST_ID` | uint32 | 反馈帧 Master ID |
| 8 | `ESC_ID` | uint32 | 电机命令 CAN ID |
| 9 | `TIMEOUT` | uint32 | CAN 超时周期，官网计量单位 50 us |
| 10 | `CTRL_MODE` | uint32 | 控制模式 |
| 11 | `Damp` | float32 | 速度环阻尼因子 |
| 13..15 | `hw_ver/sw_ver/SN` | uint32 | 硬件版本、软件版本、序列号 |
| 16 | `NPP` | uint32 | 电机极对数 |
| 17..20 | `Rs/LS/Flux/Gr` | float32 | 相电阻、相电感、磁链、减速比 |
| 21 | `PMAX` | float32 | 位置编解码量程，rad |
| 22 | `VMAX` | float32 | 速度编解码量程，rad/s |
| 23 | `TMAX` | float32 | 力矩编解码量程，N m |
| 24 | `I_BW` | float32 | 电流环带宽 |
| 25..26 | `KP_ASR/KI_ASR` | float32 | 速度环 PI 参数 |
| 27..28 | `KP_APR/KI_APR` | float32 | 位置环 PI 参数 |
| 29 | `OV_Value` | float32 | 过压保护阈值 |
| 30 | `GREF` | float32 | 齿轮力矩传递系数 |
| 35 | `can_br` | uint32 | CAN 比特率配置枚举，映射未在 SDK 中公开 |

完整枚举见
[`registers.py`](../../sdk/Gloria-M-SDK/src/gloria_m_sdk/registers.py)。`Deta`、`V_BW`、`IQ_c1`、`VL_c1`、校准偏置和 `p_m/xout` 等条目的完整语义未在当前公开资料中定义，不建议在不了解固件版本的情况下写入。

### 5.2 控制模式值

| 值 | SDK 枚举 | 可核实的协议资料 |
|---:|---|---|
| 1 | `MIT` | 有命令帧定义、发送 API、示例和测试 |
| 2 | `POS_VEL` | 有命令帧定义、发送 API、示例和测试 |
| 3 | `VEL` | 只有枚举，未找到命令帧定义、发送 API、示例或测试 |
| 4 | `TORQUE_POS` | 只有枚举，公开协议未给出足够的命令帧定义 |

SDK 的通用 `set_mode()` 可以把任意 `ControlMode` 数值写入 RID 10，但 SDK `MotionAPI` 实际只实现 `send_mit()` 和 `send_pos_vel()`。仅能写入并回读模式枚举，不代表对应运动协议已经实现或经过验证。在获得厂家完整帧定义并完成实机验证前，不应选择 `VEL` 或 `TORQUE_POS`。

SDK 的 `GloriaGripper.connect()` 默认会写入并保存量程。只读检查时应考虑使用 `connect(apply_limits=False)`；需要变更量程时，应先核对设备和主机侧参数，并只在必要时保存一次，避免无意义的 Flash 写入。

通用寄存器写入可能改变 CAN ID、保护阈值、控制环参数或通信配置，参数保存还会消耗 Flash 写入寿命。建议先在失能状态读取并记录原值，只写厂家明确支持的寄存器，写入后立即回读核对；仅在确需跨掉电保留时执行保存。PMAX、VMAX、TMAX 必须与主机编解码量程一致。

## 6. 方向、零点与机械限位标定
当前上游资料存在方向约定冲突：
- SDK `constants.py` 注释称 `0 rad` 为完全张开，正方向趋向闭合。
- SDK demos 和官方 SDK 页面通常使用 `open_q=2.5/2.77`、`close_q=0/0.003`，并约定正力矩张开、负力矩闭合。

因此不能把任一默认方向当作所有型号和装配的硬件事实。首次调试应：
1. 保持设备失能，移除夹持物并准备急停或断电手段。
2. 发送状态请求帧，确认 CAN ID、反馈量程和当前位置合理。
3. 在 PV 模式以很低速度发送一个很小的位置增量，观察实际开合方向。
4. 记录实机 `q_open` 和 `q_closed`，在控制器中设置不超过机械行程的安全位置范围。
5. MIT 力矩方向只用低幅值、短时间脉冲验证，并保留非零阻尼。
6. 未建立可靠机械基准前不要发送设零命令。该操作会改变后续所有绝对位置语义。

数值安全范围不表达哪一端是张开或闭合，也不能替代机械限位、驱动器保护、急停或碰撞检测。

## 7. 力矩反馈与夹持力估算
反馈 `torque` 是按 `TMAX` 解码的电机/减速器输出力矩估计，不是指端六维力传感器读数。连杆夹爪的近似指端夹持力依赖位置相关等效力臂：
$$
F_{grip}(q) \approx \frac{\tau_{contact}(q)}{r(q)}
$$

其中 `r(q)` 必须通过实际机构几何或标定得到。SDK 示例还会用空载基线扣除摩擦、重力和机构阻力。固定 `radius_mm=12`、示例目标力和接触阈值都只是演示初值，不是设备保证精度。更换指端、姿态、润滑状态或夹爪型号后应重新标定，并用独立测力计验证。

## 8. 推荐上电与调试流程
1. 核对实物型号，确认 24 V DC 电源的极性与供电能力，并检查 CAN 针脚和终端电阻。
2. 断电连接夹爪电源与 CAN，确认总线上没有 ID 冲突。
3. 确认夹爪与 CAN 适配器使用相同比特率。
4. 上电后保持失能，发送一次状态请求帧并解析完整反馈，确认设备号、故障码、位置、速度、力矩和温度合理。
5. 在失能状态写入目标控制模式并读取 RID 10 回读确认。
6. 读取 PMAX、VMAX、TMAX，确认主机编解码量程与设备一致。
7. 清空机械运动范围并准备急停或断电，再发送使能命令；必须通过反馈帧确认设备进入使能状态且无故障。
8. 首次运动使用低速、小位移 PV 命令，或低幅值、短时间且带非零阻尼的 MIT 命令。
9. 测试结束立即发送失能命令，并通过后续状态反馈确认设备状态。

## 9. 共总线约束

- 同一物理总线上的设备必须使用不同的活动命令 ID 和反馈 ID。
- 不同物理总线上的设备可以复用同一组 ID。
- 评估总线占用率时应计入仲裁、控制字段、CRC、帧间隔和最坏位填充，不能只计算 8 字节 payload。
- 高频传感器与夹爪共享总线时，应保守设置夹爪状态请求和命令频率，并监控丢帧与总线错误。
- `0x7FF` 是设备参数/状态请求广播 ID，多个主机或配置工具同时写参数会产生竞态。

## 10. 故障排查

| 现象 | 优先检查 |
|---|---|
| 完全无反馈 | 供电、急停、CAN_H/L、参考地、终端、CAN 比特率、`MST_ID` |
| 状态请求无响应 | `ESC_ID`、`MST_ID`、CAN 过滤配置、是否误用串口速率作为 CAN 速率 |
| 模式回读超时 | RID 10 回包 ID、固件兼容性、总线负载、是否有其他主机占用 |
| PMAX/VMAX/TMAX 不一致 | 读取设备配置；让主机编解码量程匹配固件，或在受控条件下重新配置固件 |
| 使能后立即失能或报警 | 解析反馈 `ERR`，检查欠压、过流、温度、过载、通信超时和机械卡滞 |
| 开合方向相反 | 实机重新标定 `q_open/q_closed`、力矩符号和零点，不要套用示例方向 |
| 位置/速度/力矩数值异常 | 主机与固件量程不一致，或反馈帧 ID/格式不匹配 |
| 运动抖动 | `kd`/阻尼过低、增益过高、加减速度不合适、总线丢帧或机构卡滞 |
| 夹持力估计偏差大 | TMAX、空载基线、力臂曲线、指端和型号不匹配 |
| 主机显示正常但设备报警 | 检查是否完整解析 `ERR`、`T_MOS` 和 `T_Rotor`，必要时使用厂家工具复核 |
