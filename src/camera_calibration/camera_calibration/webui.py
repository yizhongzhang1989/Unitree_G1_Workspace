"""面板的 HTTP 骨架。标准库 ``ThreadingHTTPServer``，不引 Flask。

和 ``record`` 包里那份是同一套（那边已经踩过坑了），这里独立一份而不是跨包 import：
标定包不该为了一百行工具代码去依赖采集包。

面板只是观察者 + 命令入口，逻辑都在节点里。页面 1 Hz 轮询，不上 WebSocket。
每个面板只要给出：POST 动作表，和一个负责 GET 分发的 ``route(handler, url)``。
"""
