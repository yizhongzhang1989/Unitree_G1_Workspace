...
* **平移和转角各自选参考系，四种搭配**（`arm_position_frame` / `arm_rotation_frame`，
  取值 `world` / `local`）。两条 delta 是两件事，别合并：

  | | `world`（看 WebXR 参考空间的 delta） | `local`（看手柄自身系的 delta） |
  |---|---|---|
  | 平移 | 参考空间轴直接落到躯干轴；**与握法、与你什么时候按 squeeze 都无关**，转身面对机器人往自己前方推手 = 末端往后收 | 脱掉**整个握姿**读成手柄自身轴上的量，原样搬到夹爪自身轴；“沿指向推”= 夹爪沿自己的伸出轴前伸 |
  | 转角 | delta 留在世界轴上再**左乘**锚点姿态；绕竖直轴拧手 = 绕躯干竖直轴拧夹爪 | 脱掉整个握姿换到 `gripper_base` 自身轴系再**右乘**；滚手腕 = 滚夹爪 |

  默认是 **平移 `world` + 转角 `local`**。轴怎么对上的见 `vr_teleop.py` 的
  `_TOOL_AXIS_MAP` / `_BASE_AXIS_MAP`。参考系在**离合接合那一刻锁死**，所以可以不重启
  现场换：`ros2 param set /vr_teleop arm_rotation_frame world`，松开 squeeze 再按一次即
  生效（正按着的时候换会让目标从锚点按新映射重算、凭空跳一下）。
* **`world` 的水平朝向靠头显自己校准，节点不做任何偏航摆正**。`local-floor` 参考空间的
  -Z 就是你进 VR（或长按重定位键）那一刻面朝的方向，所以要**和机器人同向站着进 VR**。
  离合接合的日志会打印朝向线在参考空间里的方向，手柄水平指向机器人正前方时读数接近
  `[0, 0, -1]` 就说明校准对了。`local` 不用校准。
