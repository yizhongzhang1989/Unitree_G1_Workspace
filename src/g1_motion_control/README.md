...
#### 零空间偏置：把三根自转轴拉回 0（`ik_null_bias`）

收肘限位只拦得住“越过奇异点回不来”，**拦不住“肘长期顶在 1.4”**：目标够不着时肘就得
伸直，而那里 `shoulder_yaw` 与 `wrist_roll` 的夹角只剩 9.8°。触发有多频繁完全取决于
遥操幅度（OU 随机游走）：

| 幅度 | 3 cm | 6 cm | **15 cm** | 20 cm |
|---|---|---|---|---|
| 肘顶限位的帧 | 0.0% | 0.4% | **16.6%** | 25.5% |
| 3↔5 夹角 < 20° 的帧 | 0.0% | 0.9% | **21.2%** | 31.7% |

手臂**前伸 20 cm 就到可达极限**（残差 72.5 mm），所以 VR 遥操必然频繁踩到。

> ⚠️ 这一节的结论曾经是反的。旧版文档写着“零空间偏置实测无收益，已删”——那个结论是在
> **6 cm 画圆**工况下得出的，而上表显示 6 cm 的触发率只有 0.9%。**当初的实验没覆盖
> VR 的真实幅度**，不是偏置没用。

做法是在**零空间**里给这三根轴加一个拉向 0 的梯度步 `b = -k q`：
...
1. **必须投影到零空间，不能直接加进 `dq`。** 同一个偏置向量的末端污染 `||J b|| = 0.30`，
   而该构型的任务残差才 0.0725 m。直接加会让偏置和任务在每次迭代里互相对抗：位形没
   改善（同轴帧 18.3%）、跟随反而变差（p50 33.9 → 37.8 mm）、耗时还更高。
2. **必须三根一起拉。** 只拉肘时偏置投影后的强度只有三根一起的 1/2.4（0.220 vs 0.528），
   实测形同没做（18.3%）。肘角在奇异构型下没法单独变 —— 要动它必须 3 和 5 陪着转。
3. **梯度取 L2 的 `-k q`，不是 L1 的 `-k sign(q)`。** 后者是常幅值步，到 0 附近必然
   chattering：实测目标完全不动时稳态每帧还抖 **0.103 rad = 5 rad/s**，3 cm 跟随 p50
   从 0.63 崩到 5.69 mm，连“拉向 0”本身都做得更差（残留 |q| 0.0233 vs 0.0055）。

#### 逐轴门控：正常工况必须零代价（`ik_null_bias_gate`）

**没有门控，这个偏置在正常工况是净负收益。** 它会把稳态残差从 0.858 顶到 1.933 mm，
越过 `ik_tol_pos`(1 mm) → 求解器**永远判不了收敛** → 稳态迭代从 0 变成跑满 10 次，
上面那张性能表的第一行就废了。

所以每根轴按**自己的 `|q|`** 平滑开门：低于 0.8 完全不偏置，高于 1.2 全额施加，中间线性
过渡。正常跟随时肘只在 `[-0.36, +0.34]`，门全关、走不算投影的快路径。

| | 稳态迭代 | 稳态残差 | 3 cm: p50 / p99 | 15 cm: 3↔5<20° / p50 / p99 |
|---|---|---|---|---|
| 关闭偏置 | 0 | 0.858 mm | 0.63 / 0.99 mm | 20.5% / 30.8 / 168 mm |
| 偏置**无门控** | **10** | **1.933 mm** | 0.85 / **2.47** mm | 9.1% / 26.1 / 140 mm |
| **偏置 + 逐轴门控** | **0** | **0.858 mm** | **0.63 / 0.99 mm** | **8.9% / 26.0 / 146 mm** |

前三项与**完全关闭偏置逐位相同**——正常工况零代价；而 15 cm 工况的收益一点没丢
（三种子：同轴帧 20.5/18.9/17.9% → 8.9/6.8/8.2%，p50 30.8/24.0/24.4 → 26.0/21.1/23.2 mm）。

门控信号取**每根轴自己的 `|q|`** 而不是统一看肘角：两者实测几乎等效（统一门控 p99 好
3~4%，逐轴同轴率和 p50 好 2~5%），选逐轴是因为 `ArmIK` 不必知道“哪个关节是肘”。
开始角别调到 1.0 以上 —— 实测 1.0~1.4 时同轴率只降到 13.5%，收益掉一半。

> 代价在**够不着**的场景：肘顶限位时稳态残差 72.5 → 73.5 mm（+1.4%）。这点劣化是偏置
> 向任务空间的“泄漏”造成的，而**泄漏正是效果的来源** —— 把投影阻尼从 λ² 调到 λ²/100，
...
**手臂高频抖动**。实测把末端目标放到够不着的前方 60 cm：

| | 稳态每帧关节增量 | 位置残差 |
|---|---|---|
| 不限幅 | **1.77 rad**（50 Hz 下就是剧烈抖动） | 0.93 m |
| 限到 0.1 m | **1.3e-14** | **0.45 m** |

注意残差也从 0.93 m 改善到 0.45 m —— 不限幅时那个“解”根本是在乱跳，连最近可达点都
找不到。姿态同理：末端翻 180° 时不限幅每帧抖 2.02 rad，限到 0.5 rad 后是 7.7e-15。

**这不是精度参数。** 正常跟随每帧才走 ~2 mm，远在上限之下，所以这一道对精度是空操作：
6 cm 画圆的残差 p50 0.79 mm / max 3.63 mm、平均迭代 3.4 次、单帧最大跳变 0.044 rad，
加不加限幅这四项数字**完全相同**；只有冷启动多用 1 次迭代（5 → 6）。也别调太小，
0.02 m 时冷启动 10 次迭代都够不到、残差 84 mm。

> 排查过的其它可能，都不是原因：零空间偏置（当时还在）关掉照样抖 1.77，偏置步本身
> 只有 0.005~0.065；迭代数改成 3 或 30 也一样。加大 `ik_damping` 到 0.2
> 确实能稳住，但那会牺牲正常跟随精度，而限幅不会。
...
实测 60 轮"推远 2.4 s → 拉回 8 s"：

| | 回不来 | 最差残差 | ±3 cm 跟随 p50 | ±8 cm 跟随 p50 |
|---|---|---|---|---|
| 纯热启动（现状之前） | **10/60** | 69.6 mm | 0.7 mm | 138.0 mm |
| 收肩 `pitch ≤ 1.6` | 2/60 | 120.2 mm | 0.7 mm | 24.9 mm |
| 收肩 `roll ≤ 1.5` | 7/60 | 69.6 mm | 0.7 mm | 138.0 mm |
| 整体改回固定种子 | 0/60 | 0.0 mm | 0.7 mm | 33.3 mm |
| **热启动 + 逃生种子** | **0/60** | **1.0 mm** | **0.7 mm** | **30.2 mm** |
...
阈值在 5 ~ 50 mm 之间都是 0/60，不敏感。取 **10 mm** 是因为它在正常跟随里**一次都不触发**
（±3 cm 轨迹触发率 0.00%，跟随 p50 与不加时逐位相同），只有真够不着或真卡住了才多花
一次求解（最长连续触发 4 帧，双臂一次 ~0.3 ms）。
...
| | 发布值单帧最大 | 超 0.3 rad 的帧 | 末端跟随 p50 |
|---|---|---|---|
| 不限速 | 0.645 rad（最坏场景实测到 2.59） | 56 / 1500 | 14.0 mm |
| **10 rad/s** | **0.200 rad** | **0** | 16.2 mm |
| 5 rad/s | 0.100 rad | 0 | 24.6 mm |
| 2.5 rad/s | 0.050 rad | 0 | 44.0 mm（明显拖后腿） |

顺带把 `~/start` 接管那一下也管住了：实测位形和 IK 首解不一致时不再是跳变，而是按这个
速率走过去。嫌快调到 5，但别超过 20——那时单帧就能走 0.4 rad，限速基本失去意义。
...
| `ik_null_bias` | 三根自转轴 0.20，其余 0 | 零空间偏置增益（长度同 `arm_joints`）。在**不移动末端**的前提下把 `shoulder_yaw`/`elbow`/`wrist_roll` 拉回 0。**治的是"肘长期顶在 1.4、两根自转轴夹角只剩 9.8°"**，收限位管不了。必须三根一起拉、必须投影到零空间。全填 0 = 关闭。见第 5 节 |
| `ik_null_bias_gate` | `[0.8, 1.2]` rad | 上面那一项的**逐轴门控**：某轴 `\|q\|` 低于开始角完全不偏置，高于全开角全额施加。**没这一道偏置在正常工况是净负收益**（稳态残差被顶到 1.933 mm > `ik_tol_pos`，永远判不了收敛、迭代跑满）。开始角别调到 1.0 以上。见第 5 节 |
...
关键点：**终点是安全的**（手臂全 0 垂下有 145 mm 余量），所以改 `passive_targets`
救不了。实测从"手臂在背后 +75°"起步、把终点分别换成 `roll±0.25/elbow0.5`、`roll±45°`、
`roll±0.35/elbow0.5`，**全部仍在路径 5~7% 处干涉**——撞发生在一开始。

真正的解法是**先侧开再走**，即现在的两段式 STAND（`stand_clear_roll` / `stand_clear_s`）。
同一组实测（腿最弯的最坏姿态下）：
...
（那个下界当初就是为了避开细长的 Gloria-M 夹爪：`elbow` 下界 0.0 时随机位姿自碰撞率
11%，0.3 时降到 7.5%）。进 RUNNING 后上肢由 IK 接管、起点就是 STAND 的终点，所以这是
个**分布外开局**问题。想让开局也落进训练分布就改成（顺序是左臂 7 轴 → 右臂 7 轴 →
左右夹爪偏心轴）：
...
* **没有可指定的臂角（arm-angle）控制**。`ik_null_bias` 只能把三根自转轴往 0 拉，
  不能让调用方指定“耘朝哪边”。
* 只在平地策略上验证过。
...
| **手臂老是伸得直直的、肘长期顶在 1.4** | 目标长期够不着（前伸 20 cm 就到极限）。确认 `ik_null_bias` 三根自转轴是 0.20 而不是 0，`ik_null_bias_gate` 是 `[0.8, 1.2]`。它只能改善不能根治 —— 跟随目标本身就得收到可达域内。见第 5 节 |
