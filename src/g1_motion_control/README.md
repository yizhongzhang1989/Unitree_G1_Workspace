...
### 求解：定阻尼 DLS，无零空间项、无线搜索、无 SVD、迭代数硬上限
...
手臂 7 自由度、任务只有 6 维，多出来的那一维完全由**种子**定：DLS 的最小范数步
天然留在种子附近。曾经为此加过零空间姿态偏置，实测下来它既治不了热启动的漂移（那是
累积现象，靠出口限速和逃生种子管），在固定种子下又完全多余（画 3 圈 6 cm 闭合圆）：

| `posture_weight` | 残差 p50 | 残差 max | 单帧最大跳变 | 绕整圈后 yaw 漂移 |
|---|---|---|---|---|
| **0（已删）** | 0.54 mm | 0.99 mm | 0.0230 rad | **+0.0022 rad** |
| 0.05（旧默认） | 0.56 mm | 0.98 mm | 0.0226 rad | −0.0018 rad |

四项指标全在噪声内，而不可达目标下的残留抖动两者**都是 0**（那是 `ik_max_step_*` 在治，
不是零空间在治）。所以整套零空间投影、`ik_rest_posture`、`ik_posture_weight`、
以及那个要单独调的投影阻尼 `ε²` 全部删掉了——每次迭代少解一个 6×6 方程。
...
└── test/
    ...
    ├── test_arm_ik.py           # 15 个 IK 用例（正逆往返 / 不可达不抖 / 单侧求解 / 整条管线不卡死不跳变）
