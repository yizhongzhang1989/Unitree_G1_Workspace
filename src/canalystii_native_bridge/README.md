...
3. 任意单侧夹爪反馈异常时，只允许该夹爪进入安全状态；只控制 Unitree 关节的整机低层输出必须继续运行。

本包直接负责前两项使用的共享 USB/CAN 数据路径，并为第三项移除底层耦合和单侧饥饿来源。第三项仍是跨包的系统安全属性：上层 gripper/controller manager 也必须隔离故障，最终必须在 `whole_body` 模式下通过“人为中断单侧夹爪反馈”的故障注入验证。本包的末端设备测试不能单独证明该项已经完成。

## 当前情况
以下结果来自 2026-07-22 的同机真硬件测试，左右分别使用 CAN0/CAN1。测试均先确认真实反馈，再显式使能；结束时先停止控制、失能双夹爪，最后关闭 bridge。

| 场景 | 左侧 | 右侧 | 结论 |
|---|---:|---:|---|
| Gloria disabled，KWR57 3 s 基线 | 1000.01 Hz，ROS/源最大 gap 9.06/7.77 ms | 1000.01 Hz，ROS/源最大 gap 8.85/7.76 ms | 短基线通过 |
| C++ 当前位姿 hold，实际 CAN TX | 99.99 Hz | 99.99 Hz | 100 Hz 且对称 |
| C++ hold 时夹爪反馈 | 109.99 Hz，源最大 gap 39.45 ms | 109.99 Hz，源最大 gap 27.99 ms | 无超过 50 ms 的断流 |
| Dashboard `0.00 <-> 2.50 rad` 往返反馈 | 103.89 Hz，源最大 gap 30.43 ms | 103.73 Hz，源最大 gap 29.27 ms | 两侧各完成 135 次换向 |
| Dashboard 往返实际 CAN TX | 93.90 Hz，最大 gap 28.37 ms | 93.73 Hz，最大 gap 28.05 ms | 对称，但 Python 定时器未达到 100 Hz |
| 往返时 KWR57 最终反馈 | 1000.87 Hz，ROS/源最大 gap 16.84/13.29 ms | 1000.87 Hz，ROS/源最大 gap 17.53/13.79 ms | 平均频率正常，严格 gap 未通过 |

目前可以确认：
- 旧路径的“左侧约 98 Hz、右侧约 10 Hz、右侧最大断流 1.155 s”没有在原生 bridge 下复现；双夹爪主动 hold 和大行程往返均保持左右对称，也没有触发 `0.5 s` feedback timeout 自动失能。
- 长时间测试关闭时，native transport 最终计数为 RX `1625905/1625891`、TX `15984/15977`，双通道 transport queue `drops=0`。KWR57 分别完成 `536642/536639` 个样本，组包器记录 `0/1` 个 dropped sequence。
- 节点可以正常停止，关闭后 CANalyst-II 仍以 `04d8:0053` 枚举，测试期间没有新的 USB reset、枚举失败或关键内核 USB 错误。

但当前还不能宣告最终验收完成：
- active Gloria 流量下，KWR57 最终 ROS 最大 gap 仍达到约 17 ms，尚未满足不超过 10 ms 的硬指标；平均约 1 kHz 不能掩盖这一点。
- Dashboard 往返是 Python 调试控制路径，请求 100 Hz 时实测约 94 Hz；需要精确 100 Hz 的生产控制应使用 C++/ros2_control 路径。
- “单侧夹爪反馈故障不停止 Unitree 整机低层输出”尚需 whole-body 故障注入实测，不能从纯末端往返测试推断为已经通过。

## 数据路径
```mermaid
flowchart LR
  USB["CANalyst-II USB"] -->|"每通道对称异步 RX"| T["单一 C++ transport"]
  T -->|"KWR57 三帧"| K["严格组包"]
  K -->|"直接发布"| W["WrenchStamped"]
  T -->|"Gloria 等普通 CAN 帧"| F["can_msgs/Frame 路由"]
  X["/can0/tx 与 /can1/tx"] --> Q["有界公平 TX 队列"]
  Q -->|"所有者线程同步 USB OUT"| USB
```

默认每通道保持 8 个异步 RX transfer，处理线程按通道轮转。TX 在每通道内保持 FIFO，并跨通道公平调度；寄存器事务和电机安全控制帧优先。实际 USB OUT 由唯一所有者线程执行同步 64 字节 bulk write，因为硬件实测已证明并发异步 OUT completion 不能可靠代表 CAN 命令已经进入对应通道的发送队列。
...
| `io_diagnostics` | `false` | 每秒输出各通道 RX/TX packet、frame 和 RX queue drop 计数 |
...
- `io_diagnostics` 周期 transport 计数，以及退出时 transport 和每台 KWR57 的最终统计；
...
当前纯软件测试结果为 `20 tests, 0 failures`，覆盖协议布局、KWR57 严格组包与重同步、左右状态独立、公平 TX 队列、配置冲突和 RX route。软件测试不会打开 USB，也不能替代主动硬件验收。
...
硬件验收至少包含：
1. Gloria disabled 基线：同时测量左右最终 `WrenchStamped`，检查最大 gap；
2. 双 Gloria active：使用 100 Hz C++ 控制，检查命令、实际 CAN TX、反馈和 JointState 的频率及最大 gap，同时继续检查双 KWR57；
3. 双夹爪实际往返：确认位置确实跨端点并完成多次换向，而不是只统计命令数量；
4. whole-body 故障注入：中断任意单侧夹爪反馈，确认该夹爪安全退出，而 Unitree 关节低层输出和另一侧设备继续运行。
